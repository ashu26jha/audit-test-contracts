...
import { useAllowedRepositories } from "@/hooks";
...
const AllowedRepositories: FC = () => {
  ...
  if (error) {
    return (
      <div className="w-full h-full flex justify-center items-center p-8 text-danger">
            ...
            {allowedRepositories.map((repo) => (
              <div key={repo.name} className="flex items-center justify-between p-1 rounded-md">
                <div className="flex items-center gap-2">
                  <Image src="/svg/repo-avatar.svg" alt="Repository" width={24} height={24} />
                  <span className="text-sm font-normal text-[#A1A1AA]">{repo.name}</span>
                  {repo.private && (
                    <Image src="/svg/private.svg" alt="Private" width={24} height={18} style={{ color: "#A1A1AA" }} />
                  )}
                </div>
              </div>
            ))}
