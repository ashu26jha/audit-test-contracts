...
const ScanStepper: React.FC = () => {
  ...
  const filteredSolidityFiles = solidityFiles.filter((file) =>
  ...
  return (
    <Card className="w-full min-h-screen flex flex-col">
      <CardHeader className="p-4 flex justify-between items-center border-t border-b border-gray-800">
        <div className="text-sm text-gray-400 flex">Dashboard <div className="mx-2">/</div> <div className="text-white">Scan Code</div></div>
        <div>
          <Button
            className="mr-2"
            onClick={handleBack}
        ...
        <div className="flex justify-center mb-12">
          {steps.map((step, index) => (
            <div key={step.label} className="flex items-center">
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center ${index + 1 === currentStep ? 'text-red-500' : 'text-gray-400'}`}
              >
                {index + 1 < currentStep ? step.selectedIcon : (index + 1 === currentStep ? step.selectingIcon : step.notSelectedIcon)}
              </div>
              <span className={`mx-2 text-sm ${index + 1 === currentStep ? 'text-[#C9A9E9]' : 'text-gray-400'}`}>{step.label}</span>
                ...
                <div className="mb-6">
                  ...
                  >
                    {repositories.map((repo) => (
                      <AutocompleteItem key={repo.name} value={repo.name} className='hover:bg-[#4F46E5] transition-colors duration-300'>
                        {repo.name}
                      </AutocompleteItem>
                    ))}
                  </Autocomplete>
            ...
            <>
              ...
              <div className="overflow-auto max-h-60">
                ...
                >
                  ...
                  <TableBody emptyContent={<div className='flex flex-col items-center'>
